...
function Ads() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        ...
        await axiosInstance.put(`/ads/${editingId}`, {
          adName: form.adName,
          adType: form.adType,
        ...
        await axiosInstance.post("/ads", data);
  ...
  const handleToggleStatus = async (ad) => {
    try {
      await axiosInstance.patch(`/ads/${ad.id}/toggle`);
      setAds(prev =>
        prev.map(a =>
          a.id === ad.id
          ...
          <div className="md:hidden flex flex-col gap-3">
            {filteredAds.map((ad) => (
              <div key={ad.id} className="rounded-xl border border-gray-200 p-3 bg-gray-50">
                ...
                <span
                  onClick={() => handleToggleStatus(ad)}
                  className={`inline-block mt-1 text-xs font-medium px-2 py-1 rounded cursor-pointer ${
                    ad.status === "active"
                      ? "bg-green-100 text-green-800"
                      : "bg-zinc-200 text-zinc-700"
                  }`}
